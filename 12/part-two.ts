class ConditionRecords {
    ...
    public unfold(): void {
        for (const row of this.rows) {
            const foldedConditions = row.conditions;
            const foldedQuantities = row.quantities;

            for (let i = 0; i < 5; i ++) {
                row.conditions = [...row.conditions, Condition.Unknown, ...foldedConditions];
                row.quantities = [...row.quantities, ...foldedQuantities];
            }
        }
    }
}
...
class ConditionRecordRow  {
    ...
    public getPermutations(): string[] {
        ...
        const permutations = ConditionRecordRow.generatePermutations(this.unknown().length, damagedUnknowns);
        const strings = [];
    ...
    private static generatePermutations(length: number, damagedUnknowns?: number): Condition[][] {
        if (length === 0) {
            return [[]];
        }

        const previousPermutations = ConditionRecordRow.generatePermutations(length - 1, damagedUnknowns);
        const permutations = [];

        for (let i = 0; i < previousPermutations.length; i++ ) {
            const current = previousPermutations[i];

            const newDamaged = [ ...current, Condition.Damaged ];
            const newOperational = [ ...current, Condition.Operational ];

            const newDamagedCount = newDamaged.reduce((accumulator, current) => {
                return current === Condition.Damaged ? accumulator + 1 : accumulator
            }, 0);

            if (damagedUnknowns && newDamagedCount <= damagedUnknowns) permutations.push(newDamaged);
            permutations.push(newOperational);
        }
