import { Direction } from "../16/lib/contraption.ts";
import { City, Path, push } from "./lib/city.ts";
...
while (queue.length !== 0) {
    ...
    if (city.isFinished(path.position) && path.directionStreak >= 4) {
    ...
    for (const direction of [Direction.Up, Direction.Right, Direction.Down, Direction.Left]) {
        ...
        if (path.directionStreak < 4 && path.position.row !== 0 && path.position.column !== 0) {
            continue;
        }
