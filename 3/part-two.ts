...
class PartNumber {
    ...
    constructor(value: number, x: number, y: number) {
        ...
        if (valueLength > 1) points.push({
            x: x + 1,
            y
        });
        if (valueLength > 2) points.push({
            x: x + 2,
            y
        });
...
for (const [i, row] of schematic.entries()) {
    ...
    const partNumberMatches = [...row.matchAll(partNumberRegex)].map(match => ({
    ...
    const gearRegex = /\*/g;
    // rome-ignore lint/style/noNonNullAssertion: <explanation>
    const  gearMatches = [...row.matchAll(gearRegex)].map(match => match.index!);
...
for (const gear of gears) {
    const adjacentPartNumbers = partNumbers.filter(partNumber => isAdjacent(partNumber, gear));
